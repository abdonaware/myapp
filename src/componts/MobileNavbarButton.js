...
export default function Button({ pageVeiw, setpageVeiw }) {
    const [isOpen, setIsOpen] = useState(false);
    const [onUse, setOnUse] = useState(0);
    const sortArray = ["SHOP", "FABRIC", "JOURNAL", "ABOUT"];
    const [isSmallScreen, setIsSmallScreen] = useState(window.outerWidth > 480);
    ...
    const list_click = (buttonNo) => {
        ...
        Handelclick(buttonNo); // Trigger the click handler
    };
    ...
    function Handelclick(buttonNO) {
        const newIsOpen = pageVeiw.slice();
        for (let i = 0; i < 4; i++) {
            newIsOpen[i] = i === buttonNO;
        }
        setpageVeiw(newIsOpen);
    }

    return (
        <div className="relative flex justify-center sm:justify-around">
